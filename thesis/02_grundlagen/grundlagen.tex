...
Most machines require an input facility for human control. Especially with the increasing spread of personal computers (\markup{PC}\abbrev{PC}{Personal Computer}) intuitive and easy to use Human machine interfaces (\markup{HMI}\abbrev{HMI}{Human Machine Interface}) were required. For a long time, the keyboard was the only viable input option for PCs and other kinds of computers. With the introduction of graphical user interfaces (\markup{GUI}\abbrev{GUI}{Graphical User Interface}), the mouse became the second main input method for PCs. Both, keyboards and mouses have the advantage of being an intuitive way to control GUIs, enabling the user to make precise inputs while requiring negligible amounts of computational power. 
