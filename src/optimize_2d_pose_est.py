...
from tools.training_callbacks.model_output_display import plot_to_image
...
def prepare_ds(name, ds, add_noise, add_empty, augment, hparams):
    ds = ds.map(depth_and_confmaps, num_parallel_calls = tf.data.experimental.AUTOTUNE)
...
def keypoint_error_metric(y_true, y_pred):
    """
    Calculates the mean error for each joint, derived from the maximum in each confmap
    :return:
    """
...
def make_skewed_mse(asymmetry_factor):
    """
    Creates a skewed loss funktion that punishes negative errors (predicted 0 instead of >0) more than positive errors
    :param asymmetry_factor: Factor for the amount of asymmetry.
    :return: loss function with the specified assymetry factor
    """
...
def make_model_illustrations(model, run_id, log_dir, labels, test_data):
    data = test_data.take(1)
    data_eager = next(iter(data))

    pred = model.predict(data_eager[0])

    twod_argmax = batched_twod_argmax(pred).numpy()

    plots = []
    for inp, y_true, y_pred, twod_skel in zip(data_eager[0], data_eager[1], pred, twod_argmax):
        fig = plt.figure(figsize = (15, 15))
        input_img = np.squeeze(inp)
        ...
        ax.set_title("y_true")
        stacked = input_img + np.sum(y_true, axis = 2)
        ax.imshow(stacked)

        i = 0
        stacked = input_img
        for pic in output:
            ax = fig.add_subplot(gs[3 + math.floor(i / 7), 1 + i % 7])
        ...
        img = tools.colorize_cv(input_img)
        ax = fig.add_subplot(gs[0:3, 6:9])
        ax.set_title("Pose Predicted")
        img = tools.render_skeleton(img, twod_skel, joint_idxs = True, joint_names = labels)
        ax.imshow(img)

        fig.tight_layout()

        plot = plot_to_image(fig)
        plots.append(plot)

        import cv2
        tools.telegram_tools.telegram_sendPhoto(telegram_user,
                                                telegram_token,
                                                telegram_chat,
                                                cv2.imencode(".png", plot.numpy())[1].tostring(),
                                                caption = "Run-ID: {} Illustration Sample: {}".format(run_id, i)
                                                )

    os.makedirs(log_dir, exist_ok = True)
    writer = tf.summary.create_file_writer(log_dir)
    with writer.as_default():
        tf.summary.image("!A Result plots", plots, step = 0, max_outputs = 100)


def make_model(hparams):
    """
    Make a CNN according to the hyperparameters in hparams
    :param hparams: Hyperparameters. May contain
                    HP_L1REGULARIZATION,
                    HP_L2REGULARIZATION,
                    HP_DECODERLAYERS,
                    HP_DROPOUT
...
def train_pose_estimator(model, train_data, validation_data, test_data, hparams, run_id, tensorboard_dir,
                         checkpoint_dir):
    """
    Run a single trial and log the result

    :param model: Model to run the trial on
    :param train_data: Train dataset
    :param validation_data: Validation dataset
    :param test_data: Testing dataset
    :param hparams: set of hyper parameters to test
    :param run_id: unique id for the current trial
    :param tensorboard_dir: logging directory
    :param checkpoint_dir: Directory the checkpoints are going to be saved in
    :return:
    """
    lr_reduce_patience = 5
    lr_reduce_limit = 0.00000001
    ...
    if hparams[HP_PRETRAINING]:
        ...
        tensorboard = tf.keras.callbacks.TensorBoard(log_dir = os.path.join(tensorboard_dir, "pretraining"),
                                                     histogram_freq = 0,
                                                     update_freq = 'batch',
...
def run(run_configs):
    """
    Run all trials, randomly selecting a config from run_configs
    :param run_configs: sets of parameters
    :return:
    """
    ...
    while len(run_configs) > 0:
        ...
        try:
            ...
            make_model_illustrations(
                    model = model,
                    run_id = run_id,
                    log_dir = os.path.join(run_logdir, "images"),
                    labels = ds_provider.joint_names,
                    test_data = ds_test
                    )
        except Exception as ex:
            logger.exception(ex)
...
if __name__ == '__main__':
    ...
    HP_LOSS = hp.HParam('lossfunc',
                        display_name = 'Loss function to be used',
                        description = 'Loss function to be used',
                        domain = hp.Discrete(['skewed', 'mse']))

    HP_PRETRAINING = hp.HParam('pretraining',
                               display_name = 'Use transfer learning step',
                               description = 'Use transfer learning step',
    ...
    METRICS = [
            ...
            hp.Metric('epoch_keypoint_error_metric', group = 'train', display_name = 'mean joint loc. error (val.)', ),
            hp.Metric('batch_accuracy', group = 'train', display_name = 'accuracy (train)', ),
            hp.Metric('batch_loss', group = 'train', display_name = 'loss (train)', ),
            hp.Metric('batch_keypoint_error_metric', group = 'train', display_name = 'mean joint loc. error (train)', )
            ]
