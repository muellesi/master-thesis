...
def _bytes_feature(value):
    ...
    if isinstance(value, tf.Tensor):
        val = [
                value.numpy()]  # BytesList won't unpack a string from an
        # EagerTensor.
...
def depth_and_confmaps(index, depth, img_width, img_height, skeleton,
                       conf_maps):
    return depth, conf_maps


@tf.function
def decode_confmaps(conf_maps):
    num_conf_maps = tf.shape(conf_maps)[0]
    images = tf.TensorArray(dtype = tf.uint16, size = num_conf_maps)
    write_idx = 0
    for m in conf_maps:
        png = tf.image.decode_png(m,
                                  channels = 1,
                                  dtype = tf.dtypes.uint16)[:, :, 0]
        images = images.write(write_idx, png)
        write_idx += 1
