...
def get_dataset(dataset_location, subset='train'):
    """"
        Returns a tf.data.DataSet for the MSRA dataset

        Dataset source: https://guiggh.github.io/publications/first-person-hands/
        "Cascaded Hand Pose Regression"
        Xiao Sun, Yichen Wei, Shuang Liang, Xiaoou Tang and Jian Sun, CVPR 2015.

        :param subset: 'train', 'validation' or 'test'
        :param dataset_location: root path to the MSRA dataset
...
def get_camera_intrinsics():
    ...
    """
    half_res_x = 640 / 2  # we scale the dpeth images up in preparation!
    half_res_y = 480 / 2  # we scale the dpeth images up in preparation!
    coeff_x = 241.42 * 2
    coeff_y = 241.42 * 2
...
def get_camera_extrinsics():
    ...
    """
    raise NotImplementedError("No extrinsics known for MSRA dataset!")
